...
def forward(x_data):
    ...
    h = F.max_pooling_2d(F.relu(model.conv1(x)), ksize = 2, stride = 2)
    h = F.max_pooling_2d(F.relu(model.conv2(h)), ksize = 3, stride = 3)
...
def draw_result(image, faces, result):
    ...
    for (x, y, w, h) in faces:
        ...
        if classNum == 0:
            cv2.putText(image, recognized_class, (x, y - 5), cv2.FONT_HERSHEY_COMPLEX, 0.4, (255,255,255))
            cv2.rectangle(image, (x, y), (x+w, y+h), (255,255,255), 3)
        elif classNum == 1:
            cv2.putText(image, recognized_class, (x, y - 5), cv2.FONT_HERSHEY_COMPLEX, 0.4, (0,0,0))
            cv2.rectangle(image, (x, y), (x+w, y+h), (0,0,0), 3)
        elif classNum == 2:
            cv2.putText(image, recognized_class, (x, y - 5), cv2.FONT_HERSHEY_COMPLEX, 0.4, (255,0,255))
            cv2.rectangle(image, (x, y), (x+w, y+h), (255,0,255), 3)
        elif classNum == 3:
            cv2.putText(image, recognized_class, (x, y - 5), cv2.FONT_HERSHEY_COMPLEX, 0.4, (255,255,0))
